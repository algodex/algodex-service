...
fn check_is_eligible(
    ...
    match check_mainnet_period(unix_time) {
        ...
        MainnetPeriod::Version2 => {
            if ownerAlgxBalance.val() < 3000 * 10u64.pow(6) {
                // FIXME change to 3000
                return false;
            }
            if *percentDistant > 0.05 {
                // 5%
                return false;
            }
            if *depth < 50.0 && matches!(orderType, Bid) {
                //FIXME
                return false;
            }
...
pub fn updateRewards(
    ...
    let StateMachine {
        ref escrowToBalance,
        ref spreads,
        ref ownerWalletToALGXBalance,
        ref mut ownerWalletAssetToRewards,
        ref algoPrice,
        ref timestep,
        ..
    } = stateMachine;
    let InitialState {
        ref escrowAddrToData,
        ref assetIdToEscrows,
        ..
    } = initialState;
    let qualityAnalytics: Vec<QualityResult> = assetIdToEscrows
        .get(inputtedAssetId)
        .unwrap()
        .iter()
        ...
        .map(|escrow| {
            ...
            let ownerAddr = &escrowAddrToData
                .get(escrow)
                .unwrap()
                .data
                .escrow_info
                .owner_addr;
            let ownerAlgxBalance =
                AlgxBalance::from(*ownerWalletToALGXBalance.get(ownerAddr).unwrap_or(&0u64));
            let spread = spreads.get(assetId);
            if spread.is_none() || spread.unwrap().ask.is_none() || spread.unwrap().bid.is_none() {
                dbg!("{spread} {assetId}");
            ...
            let orderType = match escrowAddrToData
                .get(escrow)
                .unwrap()
                .data
                .escrow_info
            ...
            let isEligible = check_is_eligible(
                &percentDistant,
                &orderType,
                &depth,
                &ownerAlgxBalance,
            ...
            return QualityResult {
                ...
                bidDepth: BidDepth::from(bidDepth),
                askDepth: AskDepth::from(askDepth),
                algxBalance: ownerAlgxBalance,
            };
        })
        .collect();

    let ownerWalletToQuality: HashMap<&String, QualityResult> = qualityAnalytics
        .iter()
        ...
        .fold(HashMap::new(), |mut ownerWalletToQuality, entry| {
            ...
            if let None = qualityDataOpt {
                ownerWalletToQuality.insert(
                    ownerAddr,
                    QualityResult {
                        addr: ownerAddr.clone(),
                        quality: Quality::from(0.0),
                        bidDepth: BidDepth::from(0.0),
                        askDepth: AskDepth::from(0.0),
    ...
    let totalBidDepth = qualityAnalytics
        .iter()
        .fold(BidDepth::from(0.0), |sum, entry| sum + entry.bidDepth);
    let totalAskDepth = qualityAnalytics
        .iter()
        .fold(AskDepth::from(0.0), |sum, entry| sum + entry.askDepth);
