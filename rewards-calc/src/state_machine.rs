...
pub fn loop_state_machine(
        ...
        && initial_state.changed_escrow_seq[state_machine.escrow_step] <= state_machine.timestep
    {
        ...
        let changed_escrows = initial_state.unix_time_to_changed_escrows.get(change_time).unwrap();
    ...
    let assets_with_balances: HashSet<&u32> =
        state_machine.escrow_to_balance.keys().fold(HashSet::new(), |mut set, escrow| {
            let asset_id =
                &initial_state.escrow_addr_to_data.get(escrow).unwrap().data.escrow_info.asset_id;
            set.insert(asset_id);
            set
        });
